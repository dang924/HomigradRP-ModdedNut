...
if (CLIENT) then
	function SWEP:PreDrawViewModel(viewModel, weapon, client)
		...
		if (hands and hands.model) then
			...
			viewModel:SetSkin(tonumber(hands.skin) or 0)
...
local function getOrganismStamina(owner)
	local organism = IsValid(owner) and owner.organism
...
local function spendPunchStamina(owner, amount)
	local displayMax = nut.config.get("defaultStamina", 100)
	local pool, current, maxStamina = getOrganismStamina(owner)

	if (pool) then
		local cost = (tonumber(amount) or 0) / displayMax * maxStamina
		...
		if (SERVER) then
			pool[1] = math.Clamp(current - cost, 0, maxStamina)
			owner:setLocalVar("stm", math.Clamp((pool[1] / maxStamina) * displayMax, 0, displayMax))
		end

		return true
	end

	local value = owner:getLocalVar("stm", 0) - amount
...
function SWEP:PrimaryAttack()
	...
	if (staminaUse > 0 and not spendPunchStamina(owner, staminaUse)) then
		return
	end
