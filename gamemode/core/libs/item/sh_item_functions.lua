NUT_ITEM_DEFAULT_FUNCTIONS = {
	...
	take = {
		...
		onRun = function(item)
			...
			-- Hold the lock for a real window (not the current instant). The old
			-- value RealTime() expired one frame later, so spamming USE across
			-- frames slipped past the guard and duplicated items (bags especially)
			-- while inventory:add was still resolving asynchronously.
			client.itemTakeTransactionTimeout = RealTime() + 1
