...
do
	...
	-- Returns true if the player is sprinting above the stamina-drain speed.
	local vectorLength2D = FindMetaTable("Vector").Length2D
	local function hasExplicitSprintIntent(client)
		local horizontalSpeed = vectorLength2D(client:GetVelocity())

		if (horizontalSpeed <= 250) then
			return false
		end

		if (client.HGMovementMode != nil) then
			return client.HGMovementMode == "sprint" and client.HGMovementSprintInput == true
		end

		if (client.HGMovementSprintIntent != nil) then
			if (client.HGMovementSprintInput != nil) then
				return client.HGMovementSprintIntent == true and client.HGMovementSprintInput == true
			end

			return client.HGMovementSprintIntent == true
		end
	...
	function playerMeta:isRunning()
		return hasExplicitSprintIntent(self)
	end
