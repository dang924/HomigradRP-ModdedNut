...
local function getKDInfo(character)
	local kills = tonumber(character and character:getData("pvpKills", 0) or 0) or 0
	local deaths = tonumber(character and character:getData("pvpDeaths", 0) or 0) or 0

	local ratio
	if (deaths <= 0) then
		ratio = kills > 0 and "INF" or "0.00"
	else
		ratio = string.format("%.2f", kills / deaths)
	end

	return kills, deaths, ratio
end
	...
	function PANEL:setup()
		...
		if (self.money) then
			local kills, deaths, ratio = getKDInfo(char)
			self.money:SetText(L("charKD", kills, deaths, ratio))

			self.money.Think = function(this)
				...
				local k, d, r = getKDInfo(activeChar)
				this:SetText(L("charKD", k, d, r))
