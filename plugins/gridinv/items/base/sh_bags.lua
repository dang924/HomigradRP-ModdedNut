...
function ITEM:onInstanced()
	-- Guard against creating a second storage inventory for the same bag.
	-- If this item already has an inventory id (e.g. onInstanced fired more than
	-- once, or after a restore race), reuse it instead of spawning an orphan
	-- duplicate inventory ("bag duplicates / loses storage" on spam-equip).
	local existing = self:getData("id")
	if (existing) then
		if (nut.inventory.instances[existing]) then
			hook.Run("SetupBagInventoryAccessRules", nut.inventory.instances[existing])
			nut.inventory.instances[existing]:sync()
			self:resolveInvAwaiters(nut.inventory.instances[existing])
			return
		end

		if (nut.inventory.loadByID) then
			nut.inventory.loadByID(existing)
